/**
 * Debug endpoint — tests DashScope connectivity and reports diagnostics
...
module.exports = async function handler(req, res) {
  ...
  results.env = {
    DASHSCOPE_API_KEY: process.env.DASHSCOPE_API_KEY 
      ? `set (${process.env.DASHSCOPE_API_KEY.slice(0, 20)}...)` 
      : "NOT SET",
    DASHSCOPE_BASE_URL: process.env.DASHSCOPE_BASE_URL || "NOT SET (fallback: dashscope-intl)",
    NEON_DATABASE_URL: process.env.NEON_DATABASE_URL ? "set" : "NOT SET",
    JWT_SECRET: process.env.JWT_SECRET ? "set" : "NOT SET",
  };
  
  // 2. Test DashScope API
  const API_BASE = process.env.DASHSCOPE_BASE_URL || "https://dashscope-intl.aliyuncs.com/compatible-mode/v1";
  const apiKey = process.env.DASHSCOPE_API_KEY;
  
  if (!apiKey) {
    results.dashscope = { error: "No API key configured" };
  ...
  try {
    ...
    if (response.ok) {
      ...
      results.dashscope.usage = data.usage;
      results.dashscope.content = data.choices?.[0]?.message?.content;
    } else {
      const errBody = await response.text();
      results.dashscope.error = errBody.slice(0, 500);
    }
  } catch (e) {
    results.dashscope = { error: e.message, stack: e.stack?.slice(0, 300) };
  }
